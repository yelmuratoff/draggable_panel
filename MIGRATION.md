...
| 3.x                                   | 4.0                                                   |
| ------------------------------------- | ----------------------------------------------------- |
| `handleBuilder`                       | `collapsedBuilder` — it _is_ the collapsed window now |
...
### Behaviour flags moved off the controller

v3 set interaction flags on the controller. In 4.0 they are one immutable
`PanelBehavior` passed to the _widget_, so a panel's configuration is visible at
its call site rather than mutated from wherever the controller was reachable.

| 3.x, on the controller | 4.0, on `DraggablePanel(behavior:)`  |
| ---------------------- | ------------------------------------ |
...
### The defaults changed, and you will notice

Three behaviours are on by default in 4.0 that had no equivalent in v3. If the
panel seems to disappear on its own, this is why:

| Default                      | What it does                                                             | Turn it off with              |
| ---------------------------- | ------------------------------------------------------------------------ | ----------------------------- |
| `idleStashDelay: 5s`         | A collapsed panel parks itself at the edge after five untouched seconds. | `idleStashDelay: null`        |
| `stashOnTapOutside: true`    | Touching the page anywhere parks a collapsed panel.                      | `stashOnTapOutside: false`    |
| `collapseOnTapOutside: true` | Touching the page closes an expanded panel.                              | `collapseOnTapOutside: false` |

For a panel that stays exactly where it is put, as v3's did:

```dart
behavior: const PanelBehavior(
  stashable: false,
  collapseOnTapOutside: false,
),
```
...
Two things the panel needs from where you mount it, both satisfied by
`MaterialApp.builder`:

- **The whole window.** Placements resolve against the window, not against the
  panel's own box, so a panel mounted inside a smaller container will position
  itself against bounds it does not occupy.
- **An ambient `Theme`.** Every visual default is derived from the surrounding
  `ColorScheme`.
